...
/*
    The buttons the Switch sees, i.e. the output side of input mapping.

    This is deliberately distinct from a physical pin (see PinId): a driver reports pins,
    config.ini maps pins onto these, and only these reach the console. The two used to be
    plain bool arrays of identical size, so nothing stopped one being indexed with the
    other's values.
...
/// One flag per Switch-facing button. 27 entries; the array this replaced was sized 36,
